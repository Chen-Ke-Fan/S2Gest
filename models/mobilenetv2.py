...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, sample_size=224, width_mult=1., in_channels=3):
...
if __name__ == "__main__":
    ...
    # 0.7 1.31M
    # 1.0 2.38M
    model = get_model(num_classes=25, sample_size=112, width_mult=1., in_channels=1)
    model = model.cuda()
    model = nn.DataParallel(model, device_ids=None)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  - Total parameters: {total_params / 1e6:.2f} M")

    input_var = Variable(torch.randn(8, 16, 1, 112, 112))
